...
var CONFIG = {  'i93_sr3'   :   {   'defaultRoute'              : true,
                                    ...
                                    'years'                     : [ 2019, 2010, 1999 ],
                                    'main_year_default'         : 2019,
                                    'years_awdt'                : [ 2019, 2010, 1999 ],
                                    'awdt_year_1_default'       : 2019,
                                    'awdt_year_2_default'       : 2010,
									
									'has_peak_hours_data'		: true,
                                    'years_peak_hours'          : [ 2019, 2010 ],
                ...
                'i90'       :   {   'defaultRoute'              : false,
                                    ...
                                    'years'                     : [ 2019, 2010 ],
                                    'main_year_default'         : 2019,
                                    'years_awdt'                : [ 2019, 2010 ],
                                    'awdt_year_1_default'       : 2019,
                                    'awdt_year_2_default'       : 2010, 

									'has_peak_hours_data'		: false,
                                    'years_peak_hours'          : [], 
                                    'peak_view_year_default'    : NaN,
                                    ...
                                    'geojsonURL' : 'data/geojson/i90_complete.geojson',                                     
...
// The 'logicalYear' parameter may be '2019, '2010', 2007, or '1999'.
...
function scrollHandler(e) {
    ...
    var searchIds = _.map(elts, 'data_id');
...
function initializeForRoute(route) {
    ...
    // Populate combo box for 'AWDT comparison' view for the current route
	...
	// If we have peak-hours data for the route...
	if (currentRoute.has_peak_hours_data === true) {
		// Enable the 'peak hour' choices in the main select_metric menu (in case they were disabled)
		$("#select_metric option[value='peak_6_to_7_am']").removeAttr('disabled');
		$("#select_metric option[value='peak_7_to_8_am']").removeAttr('disabled');
		$("#select_metric option[value='peak_8_to_9_am']").removeAttr('disabled');
		$("#select_metric option[value='cum_6_to_9_am']").removeAttr('disabled');
		$("#select_metric option[value='peak_9_to_10_am']").removeAttr('disabled');
		$("#select_metric option[value='peak_3_to_4_pm']").removeAttr('disabled');
		$("#select_metric option[value='peak_4_to_5_pm']").removeAttr('disabled');
		$("#select_metric option[value='peak_5_to_6_pm']").removeAttr('disabled');
		$("#select_metric option[value='cum_3_to_6_pm']").removeAttr('disabled');
		$("#select_metric option[value='peak_6_to_7_pm']").removeAttr('disabled');
		// Expose UI control and label to select peak-hours view
		$('#select_peak_comp_view,#select_peak_comp_view_label').show();
		// Adjust the label on the UI control to reflect the latest year for which we have peak hours data
		$('#select_peak_comp_view_label').html(currentRoute.peak_view_year_default + ' Peak Period Comparison View');
		...
		// Disable the 'peak hour' choices in the main select_metric menu
		$("#select_metric option[value='peak_6_to_7_am']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_7_to_8_am']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_8_to_9_am']").attr('disabled', 'disabled');
		$("#select_metric option[value='cum_6_to_9_am']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_9_to_10_am']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_3_to_4_pm']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_4_to_5_pm']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_5_to_6_pm']").attr('disabled', 'disabled');
		$("#select_metric option[value='cum_3_to_6_pm']").attr('disabled', 'disabled');
		$("#select_metric option[value='peak_6_to_7_pm']").attr('disabled', 'disabled');
		// Hide UI control and label to select peak-hours view
		$('#select_peak_comp_view,#select_peak_comp_view_label').hide();
	}  
...
//      results[2] - CSV data for location of town boundary lines in viz of secondary direction of route
//      results[3] - CSV data for location of town boundary lines in viz of primary direction of route
//      results[4] - GeoJSON for route (both directions, including ramps)
//
// Function: 
...
function generateViz(error, results) {
    ...
    DATA.secondaryDir_towns = results[2];
    DATA.primaryDir_towns = results[3];
    DATA.geojsonCurrentRoute = results[4];   // the GeoJSON data for the currently selected route
    ...
    function cleanupCsvRec(rec) {
        ...
        // NOTE: All routes for which we currently have data have data for 2019
        //       Currently, we only have 2010 and 1999 data for I-93/SR-3 and 2007 data for US-1.
        ...
        if (currentRoute.route === 'i93_sr3') {       
            ...
            rec.awdt_1999 = +rec.awdt_1999;
             // 2019 data - AWDT and  hourly
            rec.awdt_2019 = +rec.awdt_2019;
    ...
    // For the time bieing, we *think* the brute-force method applied below due to how lodash's _.pluck() function works.
    // Fingers crossed...
    
    
    var max_awdt = _.max([_.max(_.map(DATA.secondaryDir_data, 'awdt_1999')), 
                                      _.max(_.map(DATA.secondaryDir_data, 'awdt_2010')), 
                                      _.max(_.map(DATA.secondaryDir_data, 'awdt_2019')),
                                      _.max(_.map(DATA.secondaryDir_data, 'awdt_2019')),
                                      _.max(_.map(DATA.primaryDir_data, 'awdt_1999')), 
                                      _.max(_.map(DATA.primaryDir_data, 'awdt_2010')), 
                                      _.max(_.map(DATA.primaryDir_data, 'awdt_2019')),
                                      _.max(_.map(DATA.primaryDir_data, 'awdt_2019')) ]);                              
    widthPalettes.absolute.awdt.domain([0, max_awdt]);
    
    
    var max_hourly = _.max([ _.max(_.map(DATA.secondaryDir_data, 'peak_2019_6_to_7_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_7_to_8_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_8_to_9_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_9_to_10_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_3_to_4_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_4_to_5_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_5_to_6_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_6_to_7_pm')),                           
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_6_to_7_am')),
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_7_to_8_am')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_8_to_9_am')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_9_to_10_am')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_3_to_4_pm')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_4_to_5_pm')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_5_to_6_pm')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_6_to_7_pm')),
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_6_to_7_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_7_to_8_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_8_to_9_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_9_to_10_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2019_3_to_4_pm')), 
                                       ...
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_5_to_6_pm')), 
                                       _.max(_.map(DATA.primaryDir_data, 'peak_2019_6_to_7_pm')),
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_6_to_7_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_7_to_8_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_8_to_9_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_9_to_10_am')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_3_to_4_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_4_to_5_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_5_to_6_pm')), 
                                       _.max(_.map(DATA.secondaryDir_data, 'peak_2010_6_to_7_pm')),                           
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_6_to_7_am')), 
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_7_to_8_am')), 
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_8_to_9_am')), 
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_9_to_10_am')), 
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_3_to_4_pm')), 
                                      _.max(_.map(DATA.primaryDir_data, 'peak_2010_4_to_5_pm')), 
                                     _.max(_.map(DATA.primaryDir_data, 'peak_2010_5_to_6_pm')), 
                                     _.max(_.map(DATA.primaryDir_data, 'peak_2010_6_to_7_pm'))]);
    widthPalettes.absolute.hourly.domain([0, max_hourly]);       
    
    var max_cum = _.max([_.max(_.map(DATA.secondaryDir_data, 'cum_2019_6_to_9_am')),
                                     _.max(_.map(DATA.secondaryDir_data, 'cum_2019_6_to_9_am')), 
                                    _.max(_.map(DATA.secondaryDir_data, 'cum_2019_3_to_6_pm')),
                                    _.max(_.map(DATA.secondaryDir_data, 'cum_2019_6_to_9_am')), 
                                    _.max(_.map(DATA.secondaryDir_data, 'cum_2019_3_to_6_pm'))]);    
	...
	$('#sb_viz,#nb_viz,#eb_viz,#wb_viz').html('');
   ...
   var svgVolumeText = svgVolumeText_g
    ...
    var svgLabelText = svgLabelText_g
        ...
        .append("text")
            ...
            .attr("text-anchor", function(d, i) {
                    var retval; 
                    retval = "middle";   // This appears to work well
                    return retval;
            });
    // First line of descriptive label text
    var line1 = svgLabelText.append("tspan")
...
function symbolizeSvgWireframe(vizWireframe, divId, metric, year, color) {
    ...
    vizWireframe.volumeLines
        ...
        .style("stroke-width", function(d, i) { 
			var retval;
			if (d[attrName] == NO_DATA) {
				retval = 1; // SWAG, for now
            } else { 
				retval = widthPalette(d[attrName]);
			}
            return retval;
        });
   ...
   vizWireframe.volume_txt
	...
	// Folderol specific to I-93:
    // Machinery to hide linework for the southern HOV lane that's only present in the PM if the selected metric is for an AM time period,
